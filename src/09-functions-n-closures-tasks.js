...
function getPolynom(...args) {
  ...
  const third = [...args][2] || 0;
  function innerFunc(x) {
    switch (args.length) {
      case 3:
        return first * (x ** first) + second * x + third;
      case 2: {
...
function logger(f, g) {
  return (...args) => {
    const arr = [];
    args.forEach((item) => arr.push(JSON.stringify(item)));
    g(`${f.name}(${`${arr.join(',')}`}) starts`);
    const result = f(...args);
    g(`${f.name}(${arr.join(',')}) ends`);
    return result;
  };
}
...
function partialUsingArguments(fn, ...args1) {
  return function sum(...args) {
    return args1.concat(args).reduce((acc, curr) => acc + curr);
  };
}
